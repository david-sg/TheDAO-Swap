...
contract DaoSwap {
  ...
  uint constant MIN_WEI_VALUE = 1000000000000000;
  ...
  // constructor
  function DaoSwap(address sendEtherTo, uint tokenPriceInFinney_1000FinneyIs_1Ether, uint durationInDays) {
    TheDao = DAO(0xbb9bc244d798123fde783fcc1c72d3bb8c189413);
  ...
  function(){
      ...
      var tokens_to_send = (msg.value * WEI_PER_ETHER) / price_token;
      uint balance = TheDao.balanceOf(this);
      address payee = msg.sender;
      
      if (balance >= tokens_to_send){
        TheDao.transfer(msg.sender, tokens_to_send);
        beneficiary.send(msg.value);    
      } else {
        TheDao.transfer(msg.sender, balance);
        uint amountReturned = ((tokens_to_send - balance) * price_token) / WEI_PER_ETHER;
  ...
  function checkExpiry() afterExpiry{
      uint balance = TheDao.balanceOf(this);
      TheDao.transfer(beneficiary, balance);
  }
}
